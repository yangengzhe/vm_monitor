...
//            ip = "172.29.131.123";
    ...
    public static void sendVmStatus(String ip,float cpu,long memory,float upload,float download,long timestamp) {
        try { 
            System.out.println("本地IP:"+ip);
            sql = "select id from csp_vmitem where IP='"+ip+"'";//SQL语句  
            db1 = new DBHelper(sql);//创建DBHelper对象  
          ...
          //查找虚拟机
            ...
            if(vmid == "") return;
            //插入数据
            sql ="insert into cmp_vmstatus values(null,"+vmid+","+cpu+","+memory+","+upload+","+download+","+timestamp+",null)";
            System.out.println(sql);
            db1 = new DBHelper(sql);//创建DBHelper对象  
            db1.pst.executeUpdate();//执行语句，得到结果集
            ret.close();  
